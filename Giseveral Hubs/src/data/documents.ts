...
export interface DocItem {
  ...
  tags: string[];
  premium?: boolean;
  cover: number; // hue (0-360) for procedural cover gradient
}

export const CATEGORIES: { id: DocCategory; label: string; icon: string; description: string }[] = [
...
export const getDocById = (id: string) => DOCUMENTS.find((d) => d.id === id);
export const getDocsByCategory = (cat: DocCategory) => DOCUMENTS.filter((d) => d.category === cat);
export const getPopularDocs = (n = 6) => [...DOCUMENTS].sort((a, b) => b.downloads - a.downloads).slice(0, n);
export const getRecentDocs = (n = 6) => [...DOCUMENTS].sort((a, b) => +new Date(b.uploadedAt) - +new Date(a.uploadedAt)).slice(0, n);
