  ...
  return (
    <Layout>
      ...
      <section className="relative overflow-hidden bg-gradient-hero text-brand-foreground">
        <div className="absolute inset-0 bg-cover bg-center opacity-20" style={{ backgroundImage: `url(${n.image_url ?? newsDefaultImg})` }} />
        <div className="relative container mx-auto px-4 py-10 md:py-14 max-w-4xl">
          ...
          <div className="mt-5 flex flex-wrap items-center gap-3 text-xs">
            ...
            <span className="flex items-center gap-1 text-brand-foreground/70">
              <Calendar className="h-3 w-3" /> {new Date(n.date).toLocaleDateString("pt-PT")}
            </span>
            {n.author && <span className="text-brand-foreground/70">por {n.author}</span>}
            {(n.views ?? 0) > 0 && (
              <span className="flex items-center gap-1 text-brand-foreground/60">
                <Eye className="h-3 w-3" /> {n.views} visualizações
              </span>
            )}
          </div>
          <h1 className="mt-4 text-2xl md:text-4xl font-bold leading-tight">{n.title}</h1>
          {n.excerpt && <p className="mt-3 text-base md:text-lg text-brand-foreground/80 max-w-2xl">{n.excerpt}</p>}

          {/* Tags */}
          {n.tags && n.tags.length > 0 && (
            <div className="mt-4 flex flex-wrap gap-2">
              {n.tags.map((tag) => (
                <span key={tag} className="rounded-full bg-brand-foreground/10 px-2.5 py-0.5 text-xs text-brand-foreground/70">
                  #{tag}
                </span>
              ))}
            </div>
          ...
          <div className="mt-5 flex gap-2">
            <a href={`https://wa.me/?text=${encodeURIComponent(n.title + "\n" + pageUrl)}`}
              ...
              className="flex items-center justify-center h-8 w-8 rounded-md bg-[#25D366]/20 hover:bg-[#25D366]/30 transition-smooth"
              title="WhatsApp"
            >
            ...
            <a href={`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(pageUrl)}`}
              target="_blank" rel="noopener noreferrer"
              className="flex items-center justify-center h-8 w-8 rounded-md bg-[#1877F2]/20 hover:bg-[#1877F2]/30 transition-smooth"
              title="Facebook"
            >
              <Share2 className="h-3.5 w-3.5 text-[#1877F2]" />
            </a>
            <button onClick={copyLink}
              className="flex items-center justify-center h-8 w-8 rounded-md bg-brand-foreground/10 hover:bg-brand-foreground/20 transition-smooth"
              title="Copiar link"
            >
              {copied ? <ThumbsUp className="h-3.5 w-3.5 text-gold" /> : <Copy className="h-3.5 w-3.5 text-brand-foreground/70" />}
            </button>
          </div>
        </div>
      </section>
      ...
      {n.image_url && (
        <section className="container mx-auto px-4 -mt-6 max-w-4xl">
          <img src={n.image_url} alt={n.title} className="w-full rounded-2xl shadow-elegant object-cover max-h-80" />
        </section>
      )}
      ...
      <article className="container mx-auto px-4 py-10 max-w-3xl space-y-6 prose prose-sm prose-headings:text-brand prose-a:text-gold text-foreground/85">
        {n.content_rich ? (
          ...
          (n.content ?? []).map((p, i) => (
            <p key={i} className="leading-relaxed" data-index={i}>{p}</p>
          ))
        )}

        {/* Related Scholarship CTA */}
        {relatedScholarship && (
          <div className="mt-8 rounded-2xl border border-gold/30 bg-gold/5 p-5">
            <p className="text-xs font-semibold uppercase tracking-widest text-gold mb-2">Bolsa relacionada</p>
            <div className="flex items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                <span className="text-3xl">{relatedScholarship.flag}</span>
                <div>
                  <p className="font-bold text-brand">{relatedScholarship.title}</p>
              ...
              <Link to="/hub/bolsas/$id" params={{ id: relatedScholarship.id }}
                className="shrink-0 inline-flex items-center gap-1.5 rounded-lg bg-gradient-brand px-4 py-2 text-xs font-bold text-brand-foreground shadow-card"
              >
