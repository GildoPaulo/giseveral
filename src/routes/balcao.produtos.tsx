...
import {
  ...
  X, Loader2, Package, ImageOff, ToggleLeft, ToggleRight, ChevronDown, Upload,
} from "lucide-react";
...
function BalcaoProdutos() {
  ...
  async function handleImageFile(file: File) {
    ...
    if (error) { toast.error("Erro no upload da imagem"); setImgUploading(false); return; }
    const { data: { publicUrl } } = supabase.storage.from("service-uploads").getPublicUrl(path);
    fieldChange("image_url", publicUrl);
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  className="mt-1 w-full rounded-md border border-border bg-muted/30 px-3 py-2 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-brand/30"
                />
              </div>
              ...
              {/* Row 5: Image */}
              <div>
                <label className="text-xs font-semibold text-foreground uppercase tracking-wide">Imagem do Produto</label>
                <div className="mt-1 flex gap-2 items-center">
                  <input
                    value={form.image_url}
                    onChange={(e) => fieldChange("image_url", e.target.value)}
                    placeholder="https://... ou carrega um ficheiro →"
                    className="flex-1 rounded-md border border-border bg-muted/30 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand/30"
                  />
                  <input
                    ref={fileInputRef}
                    type="file"
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 rounded-md border border-border bg-muted px-3 py-2 text-xs font-medium hover:bg-muted/70 disabled:opacity-60 shrink-0"
                  >
                ...
                {form.image_url && (
                  <div className="mt-2 flex items-center gap-2">
                    <img src={form.image_url} alt="preview" className="h-16 w-16 rounded-md object-cover border border-border" onError={(e) => (e.currentTarget.style.display = "none")} />
                    <button type="button" onClick={() => fieldChange("image_url", "")} className="text-xs text-destructive hover:underline">Remover</button>
                  </div>
                )}
              </div>
