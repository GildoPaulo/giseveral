  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-center text-center px-14 pt-10 pb-7" style={{ borderBottom: `2px solid ${P}` }}>
        ...
        <div style={{ fontSize: 28, fontWeight: 700, letterSpacing: "-0.5px", lineHeight: 1.2 }}>{personal.nome || "Seu Nome"}</div>
        {personal.titulo && (
          <div style={{ fontSize: 13, color: P, marginTop: 4, fontWeight: 500 }}>{personal.titulo}</div>
        )}
        <div className="flex flex-wrap gap-x-4 gap-y-1 justify-center mt-3" style={{ fontSize: 10, opacity: 0.7 }}>
        ...
        <div className="flex flex-col gap-6 px-7 py-7" style={{ width: "38%", background: "#f8f9fb", borderRight: "1px solid #e5e7eb" }}>
          {objetivo && (
            <div>
              ...
              <p style={{ fontSize: 10, lineHeight: 1.6, opacity: 0.75 }}>{objetivo}</p>
            </div>
          )}
          {sections.educacao.visible && educacao.length > 0 && (
            <div>
              ...
              <div className="flex flex-col gap-3">
                {educacao.map(e => (
                  <div key={e.id}>
                    <div style={{ fontWeight: 600, fontSize: 10 }}>{e.grau}</div>
                    <div style={{ fontSize: 10, color: P }}>{e.instituicao}</div>
                    {e.curso && <div style={{ fontSize: 9, opacity: 0.6 }}>{e.curso}</div>}
          ...
          {sections.idiomas.visible && idiomas.length > 0 && (
            <div>
              ...
              <div className="flex flex-col gap-1.5">
                {idiomas.map(id => (
                  <div key={id.id} className="flex justify-between" style={{ fontSize: 10 }}>
                  ...
                  {experiencia.map(exp => (
                    <div key={exp.id} style={{ position: "relative" }}>
                      ...
                      <div className="flex justify-between items-start">
                        <div>
                          <div style={{ fontWeight: 700, fontSize: 11 }}>{exp.cargo}</div>
                          <div style={{ fontSize: 10, color: P, fontWeight: 500 }}>{exp.empresa}</div>
                      ...
                      {exp.descricao && <p style={{ fontSize: 10, lineHeight: 1.6, marginTop: 4, opacity: 0.75 }}>{exp.descricao}</p>}
                    </div>
                  ))}
                </div>
              </div>
                ...
                {projetos.map(proj => (
                  <div key={proj.id}>
                    <div className="flex justify-between items-baseline">
                      <span style={{ fontWeight: 600, fontSize: 11 }}>{proj.nome}</span>
                    ...
                    {proj.descricao && <p style={{ fontSize: 10, lineHeight: 1.6, marginTop: 3, opacity: 0.75 }}>{proj.descricao}</p>}
                  </div>
                ))}
              </div>
            </div>
...
function SectionHead({ label, color }: { label: string; color: string }) {
  return (
    <div style={{ fontSize: 9, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.12em", color, marginBottom: 8, display: "flex", alignItems: "center", gap: 6 }}>
