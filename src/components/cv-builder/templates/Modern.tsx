  ...
  return (
    ...
    >
      ...
      >
        ...
        <div>
          <div style={{ fontSize: s(18), fontWeight: 800, lineHeight: 1.25, letterSpacing: "-0.3px", ...longTextStyle }}>{personal.nome || "Seu Nome"}</div>
          {personal.titulo && <div style={{ fontSize: s(10.5), marginTop: 4, opacity: 0.75, fontWeight: 400 }}>{personal.titulo}</div>}
        </div>
        ...
        {sections.skills.visible && skills.length > 0 && (
          <div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 7, marginTop: 8 }}>
              {skills.map(sk => (
                <div key={sk.id}>
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: s(9.5), marginBottom: 2, opacity: 0.9, gap: 8, ...flexColumnStyle }}>
                    <span>{sk.nome}</span>
                    <span style={{ opacity: 0.6, fontSize: s(8.5) }}>{sk.nivel}</span>
                  </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 5, marginTop: 8 }}>
              {idiomas.map(id => (
                <div key={id.id} style={{ display: "flex", justifyContent: "space-between", fontSize: s(10) }}>
                  <span>{id.idioma}</span>
                  <span style={{ opacity: 0.6, fontSize: s(9) }}>{id.nivel}</span>
                </div>
              ))}
            </div>
          </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: 8 }}>
              {certificacoes.map(cert => (
                <div key={cert.id}>
                  ...
                  <div style={{ fontSize: s(9), opacity: 0.6 }}>{cert.emissor}</div>
                  <div style={{ fontSize: s(9), opacity: 0.5 }}>{cert.data}</div>
                </div>
              ))}
            </div>
          </div>
      ...
      {/* ── MAIN ── */}
      <div style={{ flex: 1, padding: "40px 40px", display: "flex", flexDirection: "column", gap: 22, ...flexColumnStyle }}>
        {objetivo && (
...
function SLabel({ label }: { label: string }) {
  return <div style={{ fontSize: s(8), fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.15em", opacity: 0.5 }}>{label}</div>;
}
...
function CI({ icon, text }: { icon: string; text: string }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 5, fontSize: s(9.5), opacity: 0.8 }}>
      <span style={{ opacity: 0.6, fontSize: s(9), width: 10, textAlign: "center" }}>{icon}</span>
